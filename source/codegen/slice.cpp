	...
	fir::Value* beginIndex, fir::Value* endIndex, sst::Expr* bexpr, sst::Expr* eexpr)
{
	...
	fir::Value* slice = cs->irb.CreateValue(slct);

	// FINALLY.
	// increment ptr
	fir::Value* newptr = cs->irb.CreatePointerAdd(data, beginIndex);
	fir::Value* newlen = cs->irb.CreateSub(endIndex, beginIndex);
...
CGResult sst::SliceOp::_codegen(cgn::CodegenState* cs, fir::Type* infer)
{
...

	{
		...
		else			this->cgEnd = cs->irb.CreateGetDynamicArrayLength(lhs);
	...
	if(ty->isDynamicArrayType())
	{
		...
		return performSliceOperation(cs, this, ty->getArrayElementType(), cs->irb.CreateGetDynamicArrayData(lhs),
			cs->irb.CreateGetDynamicArrayLength(lhs), this->cgBegin, this->cgEnd, this->begin, this->end);
	}
	else if(ty->isArrayType())
	{
		...
		return performSliceOperation(cs, this, ty->getArrayElementType(), data,
			fir::ConstantInt::getInt64(ty->toArrayType()->getArraySize()), this->cgBegin, this->cgEnd, this->begin, this->end);
	}
	else if(ty->isArraySliceType())
	{
		return performSliceOperation(cs, this, ty->getArrayElementType(), cs->irb.CreateGetArraySliceData(lhs),
			cs->irb.CreateGetArraySliceLength(lhs), this->cgBegin, this->cgEnd, this->begin, this->end);
	}
	else if(ty->isStringType())
	{
		...
		checkSliceOperation(cs, this, cs->irb.CreateGetStringLength(lhs), this->cgBegin, this->cgEnd, this->begin, this->end);


		// ok
		fir::Value* srcptr = cs->irb.CreatePointerAdd(cs->irb.CreateGetStringData(lhs), this->cgBegin);
		fir::Value* length = cs->irb.CreateSub(this->cgEnd, this->cgBegin);

		fir::Value* data = 0;
		{
			// space for null + refcount
			size_t i64Size = 8;
			fir::Value* malloclen = cs->irb.CreateAdd(length, fir::ConstantInt::getInt64(1 + i64Size));
			...
			cs->irb.CreateCall(memcpyf, { data, srcptr, cs->irb.CreateIntSizeCast(length, fir::Type::getInt64()),
				fir::ConstantInt::getInt32(0), fir::ConstantBool::get(false) });

			// null terminator
			cs->irb.CreateStore(fir::ConstantInt::getInt8(0), cs->irb.CreatePointerAdd(data, length));
		}

		// ok, now fix it
		fir::Value* str = cs->irb.CreateValue(fir::StringType::get());
		str = cs->irb.CreateSetStringData(str, data);
		str = cs->irb.CreateSetStringLength(str, length);
