...
namespace fir
{
	...
	bool checkLiteralFitsIntoType(fir::PrimitiveType* target, mpfr::mpreal num)
	{
		...
		{
			if(target == fir::Type::getFloat32())		return (num >= mpr(__FLT_MIN__) && num <= mpr(__FLT_MAX__));
			else if(target == fir::Type::getFloat64())	return (num >= mpr(__DBL_MIN__) && num <= mpr(__DBL_MAX__));
			else if(target == fir::Type::getFloat80())	return (num >= mpr(__LDBL_MIN__) && num <= mpr(__LDBL_MAX__));
