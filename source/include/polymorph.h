...
namespace sst
{
	...
	namespace poly
	{
		...
		{
			Solution_t() { }
			explicit Solution_t(const ska::flat_hash_map<std::string, fir::Type*>& p) : solutions(p) { }
			...
			int distance = 0;
			ska::flat_hash_map<std::string, fir::Type*> solutions;
			ska::flat_hash_map<fir::Type*, fir::Type*> substitutions;
		};
		...
		namespace internal
		{
			...
			std::pair<Solution_t, ErrorMsg*> inferTypesForPolymorph(TypecheckState* fs, ast::Parameterisable* thing, const std::string& name,
				...
				fir::Type* return_infer, fir::Type*, bool isFnCall, fir::Type* problem_infer, ska::flat_hash_map<std::string, size_t>* origParamOrder);
		}
	}
}
