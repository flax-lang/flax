				...
				*/
				...
				if(expt->isNamedType())
				{
					...
					if(needsolve)
					{
						...
						{
							iceAssert("how did this match" && 0);
						}
					}
					else
					{
						// if we don't need to solve it, then...
						// since 'tosolve' contains *all* the generic types at the top level, regardless of whether they
						// already have a solution,

						// we can thus conclude that it must be a normal, non-generic type that we have on our hands here.
						// so, let's get it

						fir::Type* actual = cgi->getTypeFromParserType(candidate, expt);
						iceAssert(actual);

						// check that the types work -- previously we didn't check this, so there was a hole where trying to match say
						// a tuple, against 'int' wasn't caught, because 'int' didn't "need to be solved", so to speak.

						// we first need to see if there's an existing solution for it. If not, then we can set the solution instead.
						if(givent->isParametricType())
						{
